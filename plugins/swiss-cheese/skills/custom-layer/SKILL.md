...
- **agents**: create `.claude/agents/review-<id>.md` in the project, modeled on the plugin's review agents: frontmatter with `tools: Read, Grep, Glob` and `memory: project` (persistent memory in `.claude/agent-memory/<name>/` — the agent accumulates project decisions and false-positive patterns across sessions); body = one lens, the shared-diff input protocol ("Read diff.patch from the given path; open sources only to confirm"), the `FINDING: severity | file:line | issue | fix` output contract, "stay in your lane", and a memory protocol paragraph (consult before, record durable knowledge after, never store secrets, project files read-only). Then add the id to the review layer's `agents` list. Warn: each agent slice costs tokens per review — the lens must pay for itself.
